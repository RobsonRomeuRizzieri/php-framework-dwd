	...
	************************************************************************************************************/
	...
	var tabView_maxNumberOfTabs = 26;	// Maximum number of tabs
  ...
  function definir_atalhos_consulta(tab){
    shortcut.remove("ctrl+shift+n");
		shortcut.add("ctrl+shift+n",function()
		{
       document.getElementById('frm_con_nov'+tab).click(); return false;
		});
		shortcut.remove("ctrl+shift+r");
			shortcut.add("ctrl+shift+r",function()
		{
       document.getElementById('frm_con_gra'+tab).click(); return false;
		});  }

  function definir_atalhos_filtro(tab){    shortcut.remove("ctrl+shift+f");
		shortcut.add("ctrl+shift+f",function()
		{
       document.getElementById('frm_fil_fil'+tab).click(); return false;
		});
		shortcut.remove("ctrl+shift+l");
		shortcut.add("ctrl+shift+l",function()
		{
       document.getElementById('frm_fil_lim'+tab).click(); return false;
		});  }

  function definir_atalhos_cadastro(tab){    shortcut.remove("ctrl+shift+n");
		shortcut.add("ctrl+shift+n",function()
		{
       document.getElementById('frm_cad_nov'+tab).click(); return false;
		});
		shortcut.remove("ctrl+shift+r");
		shortcut.add("ctrl+shift+r",function()
		{
       document.getElementById('frm_cad_gra'+tab).click(); return false;
		});
		shortcut.remove("ctrl+shift+a");
		shortcut.add("ctrl+shift+a",function()
		{
       document.getElementById('frm_cad_alt'+tab).click(); return false;
		});
		shortcut.remove("ctrl+shift+c");
		shortcut.add("ctrl+shift+c",function()
		{
       document.getElementById('frm_cad_con'+tab).click(); return false;
		});  }
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src = '../dwd_img/tab_right_inactive.gif';
		...
		img.src = '../dwd_img/tab_right_active.gif';
		...
		var frm_tipo = arr_frm_tipo[tabIndex];
		if (frm_tipo ==  0){
      definir_atalhos_consulta('tabView'+parentId+'_'+tabIndex);
      definir_atalhos_filtro('tabView'+parentId+'_'+tabIndex);
    }else{
      definir_atalhos_cadastro('tabView'+parentId+'_'+tabIndex);
    }
		return;
	}
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = '../dwd_img/tab_right_over.gif';
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			//alert('tabTab' + mainContainerID + "_" +  (no + tabView_countTabs[mainContainerID]));
			...
			if(closeButtonArray[no]){
				...
				closeButton.src = '../dwd_img/close.gif';
	...
	function resetTabIds(parentId,label)
	{
    ...
    var tab_temp = 0;
		...
		for(var no=0;no<divs.length;no++){			if(divs[no].className=='dhtmlgoodies_aTab'){
			...
			if(divs[no].id.indexOf('tabTab')>=0){
				indice_antigo = divs[no].id.replace('tabTab' + parentId + '_', '');
		    arr_descricao_tab[tabContentCounter] = arr_descricao_tab[indice_antigo];
				divs[no].id = 'tabTab' + parentId + '_' + tabContentCounter;
				tabContentCounter++;
			}
		}
		tab_temp = arr_descricao_tab.pop();
	...
	{
	  var indice = 0;
 		for(var rrr=0;rrr<arr_descricao_tab.length;rrr++){
      if (arr_descricao_tab[rrr] == tabTitle ){        indice = rrr;
      }
    }
	  if (arr_descricao_tab[indice] == tabTitle ){      showTab(parentId,indice);
	  }else{
			...
			arr_descricao_tab[tabId] = tabTitle;
			if(tabContent)div.innerHTML = tabContent;
			if(tabContentUrl){
				var ajaxIndex = ajaxObjects.length;
				ajaxObjects[ajaxIndex] = new sack();
				//alert('tabView'+parentId + '_' + tabId);
				//Define arquivo a ser aberto e o nome da tab a ser usada usando o parametro dwd_tab
				ajaxObjects[ajaxIndex].requestFile = tabContentUrl+'&dwd_tab=tabView'+parentId + '_' + tabId;	// Specifying which file to get
