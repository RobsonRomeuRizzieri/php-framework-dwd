...
function treeview(_parent){
	...
	this.add = function(_texto, _link, _target, _nodePai){
		...
		noh.className = 'node';
		if (_link != ''){
      adicionaEvento(noh, function() { createNewTab('dhtmlgoodies_tabView2',_texto,'',_link+'?a=1',true); }, 'click');
    }
		if (_nodePai == null){
	...
	this.addLoader = function(_parametros, _nodePai){
		...
		nodePaiLk = document.getElementById(this.id + '_divLinks' + nodePai.id.substring(this.id.length + 9));

		nohText = document.createElement("SPAN");
		nohText.innerHTML = "Aguarde, carregando...";
		nohText.className = "avisoCarregandoTreeview";
		nohText.style.display = "block";
		document.getElementById(noh.id).appendChild(nohText);

		var linkNId = this.id + '_divLinkNodes' + nodePai.id.substring(this.id.length + 9);

		if (document.getElementById(linkNId) == undefined){
			linkN = document.createElement("A");
			linkN.id = linkNId;
			linkN.className = "linkAbreFechaNode";
			linkN.href = "javascript:carregaNodes('" + nodePai.id + "','" + _parametros + "','" + linkNId + "');";
			nodePai.insertBefore(linkN, nodePaiLk);
		}
...
function carregaNodes(_node, _parameters, _linkNid){
	...
	openNode(_node);
	ajaxLoadNodes('geracaoTreeview.gm?' + _parameters + '&pai=' + _node, _node);
}
