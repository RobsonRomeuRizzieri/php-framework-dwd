...
function wp_codebox_highlight_geshi($match)
{
  ...
  if ($match[1]) {$language = strtolower(trim($match[1]));} else {$language = "text";}
  $line = trim($match[4]);
  $file = strtolower(trim($match[2]));
  $colla = trim($match[3]);
  ...
  if (!($cb_plain_txt)){
	  $output .= "<div class=\"wp_codebox_msgheader";
		if (($cb_colla && (!($colla == "+"))) || ($colla == "-")){$output .= " wp_codebox_hide";}	  
	  $output .= "\">";
	  ...
	  if ($file) {$output .= "<span class=\"left2\">".__('Download','wp-codebox').' <a href="'
	              . get_bloginfo('wpurl')
	              . '/wp-content/plugins/wp-codebox/wp-codebox.php?p=' . $post->ID
	              . '&amp;download=' . wp_specialchars($file)
	              . '">' . wp_specialchars($file) . '</a>';}

	  else {$output .= "<span class=\"left\">"
	  			. "<a href=\"javascript:;\" onclick=\"javascript:showCodeTxt('p"
	  			. $post->ID."code".$codeid
	  			. "'); return false;\">".__('View Code','wp-codebox')."</a> "
	  			. strtoupper($language);}
...
function wp_codebox_before_filter($content)
{
  return preg_replace_callback(
      "/\s*<pre(?:lang=[\"']([\w-]*)[\"']|file=[\"']([\w-]*\.?[\w-]*)[\"']|colla=[\"']([\+\-])[\"']|line=[\"'](\d*|n)[\"']|\s)+>(.*)<\/pre>\s*/siU",
...
function wp_codebox_after_filter($content)
{
  ...
  $content = preg_replace_callback(
       "/<p>\s*".$wp_codebox_token."(\d{3})\s*<\/p>/si",
       "wp_codebox_highlight",
       $content
  );
